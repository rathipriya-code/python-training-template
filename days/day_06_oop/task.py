...
class TimesheetEntry:
    ...
    def get_summary(self) -> str:
        ...
        """
        return (
            f"Summary: {self.entry_date}: {self.consultant.name} on "
            f"{self.project.code}"
            f"({self.hours} hours)"
        )
...
class TimesheetManager:
    ...
    def get_entries_by_consultant(self, consultant_name: str) -> List[TimesheetEntry]:
        ...
        """
        return [
            entry for entry in self.entries if entry.consultant.name == consultant_name
        ]
